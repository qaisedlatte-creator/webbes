  ...
  return (
    <div style={{ background: BG, color: N, minHeight: '100vh' }}>
      ...
      <header style={{ padding: 'clamp(100px,12vw,160px) clamp(20px,5vw,64px) clamp(72px,9vw,120px)', maxWidth: 1200, margin: '0 auto' }}>
        {/* Label — animates on mount */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, ease: 'easeOut' }}
          style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 28, fontSize: '0.78rem', fontWeight: 600, color: N45, letterSpacing: 1 }}
        >
          <span style={{ width: 7, height: 7, borderRadius: '50%', background: '#22c55e', display: 'inline-block' }} />
          Now Accepting New Projects
        </motion.div>

        {/* Headline — animates on mount */}
        <motion.h1
          initial={{ opacity: 0, y: 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6, ease: 'easeOut', delay: 0.15 }}
          style={{ fontFamily: 'var(--font-plus-jakarta)', fontSize: 'clamp(3rem,7vw,5.5rem)', fontWeight: 800, lineHeight: 1.05, letterSpacing: '-2px', color: N, marginBottom: 24 }}
        >
          Websites. AI.<br />Results.
        </motion.h1>

        {/* Subtitle — animates on mount */}
        <motion.p
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, ease: 'easeOut', delay: 0.3 }}
          style={{ fontSize: '1rem', lineHeight: 1.72, color: N70, maxWidth: 520, marginBottom: 32 }}
        >
      ...
      <section className="border-t border-neutral-200 py-24 px-6 bg-[#FAF9F6]">
        <div className="max-w-5xl mx-auto">
          {/* Heading block animates in — slider itself does not */}
          <motion.div
            className="mb-12"
            initial={{ opacity: 0, y: 30 }}
      ...
      <section className="border-t border-neutral-200 py-24 px-6 bg-[#FAF9F6]">
        <div className="max-w-5xl mx-auto">
          <motion.p
            className="text-[11px] tracking-[0.25em] text-neutral-400 uppercase mb-16"
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={VP}
            transition={{ duration: 0.6, ease: 'easeOut' }}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
          {SERVICES.map((s, i) => (
            <motion.div
              ...
              viewport={VP}
              transition={{ duration: 0.5, ease: 'easeOut', delay: i * 0.1 }}
              style={{ background: BG_CARD, border: `1px solid ${BORDER}`, borderRadius: 20, padding: '32px 28px', position: 'relative' }}
            >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 20 }}>
          {WHY.map((w, i) => (
            <motion.div
              ...
              viewport={VP}
              transition={{ duration: 0.5, ease: 'easeOut', delay: i * 0.1 }}
              style={{ background: BG_CARD, border: `1px solid ${BORDER}`, borderRadius: 20, padding: '32px 28px' }}
            >
        ...
        <div style={{ maxWidth: 720, margin: '0 auto' }}>
          {FAQS.map((f, i) => (
            <motion.div
              ...
              viewport={VP}
              transition={{ duration: 0.5, ease: 'easeOut', delay: i * 0.1 }}
              style={{ borderTop: `1px solid ${BORDER}`, ...(i === FAQS.length - 1 ? { borderBottom: `1px solid ${BORDER}` } : {}) }}
            >
              <button
      ...
      <section style={{ padding: 'clamp(72px,9vw,120px) clamp(20px,5vw,64px)', maxWidth: 1200, margin: '0 auto', borderTop: `1px solid ${BORDER}` }}>
        <motion.div
          style={{ textAlign: 'center', maxWidth: 600, margin: '0 auto', padding: 'clamp(40px,6vw,72px) clamp(20px,5vw,48px)', border: `1px solid ${BORDER_STRONG}`, borderRadius: 20, background: BG_CARD }}
          initial={{ opacity: 0, y: 30 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={VP}
          transition={{ duration: 0.6, ease: 'easeOut' }}
